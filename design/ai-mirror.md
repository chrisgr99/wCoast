# LibreSynth patch mirror — Design

The **patch mirror** projects the state of the running LibreSynth app to a folder of
...
settings — by writing files that LibreSynth validates and applies.

This is the LibreSynth equivalent of GeoSonel's composition mirror (GXW DESIGN §15;
`~/ProgrammingProjects/GXW/mirror-docs/AGENTS.md` and `electron-mirror.js`). It
adopts GXW's architecture and its hard-won lessons; it differs where LibreSynth is
simpler (one running patch, no moving physics) and where LibreSynth is richer (a
...
**Round-trip** (the AI may write these; LibreSynth validates and applies them):
...
**Observation-only** (LibreSynth writes; the AI reads):
...
- `catalogue.json` — the machine-readable authoring schema: every registered
  ...
  from the descriptors, so it is always exact and never drifts. (LibreSynth's answer
...
The most important AI-facing file, and the piece LibreSynth can do better than GXW.
LibreSynth modules are already fully described by their descriptors (params carry
...
- **Atomic temp-and-rename.** LibreSynth writes `patch.json.tmp` then renames to
...
store, since LibreSynth has none yet). The toggle lives in the hamburger menu — a new
...
LibreSynth projects which module they mean: the module last under the pointer, written
...
LibreSynth has no discrete musical events to log the way GXW does; it has continuous
...
- **Onsets** — amplitude-envelope peaks on the gate/voice outputs, which recreate
  ...
  centroid }`. This is the closest LibreSynth comes to GXW's event trace, and it lets
...
- **No moving physics.** GXW's `runtime-state.json` captures sprite positions and
  cursor sweeps; LibreSynth's runtime state is nearly nothing (VU + on/off), so it is
  deferred and tiny.
