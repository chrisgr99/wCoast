# LibreModular patch mirror — Design

The **patch mirror** projects the state of the running LibreModular app to a folder of
...
settings — by writing files that LibreModular validates and applies.

This is the LibreModular equivalent of GeoSonel's composition mirror (GXW DESIGN §15;
`~/ProgrammingProjects/GXW/mirror-docs/AGENTS.md` and `electron-mirror.js`). It
adopts GXW's architecture and its hard-won lessons; it differs where LibreModular is
simpler (one running patch, no moving physics) and where LibreModular is richer (a
...
**Round-trip** (the AI may write these; LibreModular validates and applies them):
...
**Observation-only** (LibreModular writes; the AI reads):
...
- `catalogue.json` — the machine-readable authoring schema: every registered
  ...
  from the descriptors, so it is always exact and never drifts. (LibreModular's answer
...
The most important AI-facing file, and the piece LibreModular can do better than GXW.
LibreModular modules are already fully described by their descriptors (params carry
...
- **Atomic temp-and-rename.** LibreModular writes `patch.json.tmp` then renames to
...
store, since LibreModular has none yet). The toggle lives in the hamburger menu — a new
...
LibreModular projects which module they mean: the module last under the pointer, written
...
LibreModular has no discrete musical events to log the way GXW does; it has continuous
...
- **Onsets** — amplitude-envelope peaks on the gate/voice outputs, which recreate
  ...
  centroid }`. This is the closest LibreModular comes to GXW's event trace, and it lets
...
- **No moving physics.** GXW's `runtime-state.json` captures sprite positions and
  cursor sweeps; LibreModular's runtime state is nearly nothing (VU + on/off), so it is
  deferred and tiny.
