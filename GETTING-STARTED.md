...
11. **Focusing a large patch.** Hover a module — or a single mixer channel — to light just the cables that feed it and that it feeds; isolate a terminal's subnet to work on one branch, with terminals pulsing in time with their signal. Positions LibreSynth for *understanding* a patch, not only building one.
