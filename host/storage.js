...
function browserStorage() {
  ...
  const PICKER = { types: [{ description: 'DreamRack Patch', accept: { 'application/json': ['.drack', '.wcoast'] } }] };
  ...
  async function saveAs(text) {
    const h = await orNull(() => window.showSaveFilePicker({ ...PICKER, suggestedName: (current && current.name) || 'patch.drack', startIn: current || 'documents' }));
    if (!h) return null;
    await write(h, text);
    await remember(h);
    return h.name;
