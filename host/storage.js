...
function browserStorage() {
  ...
  const PICKER = { types: [{ description: 'LibreSynth Patch', accept: { 'application/json': ['.lsyn'] } }] };
  ...
  async function saveAs(text) {
    const h = await orNull(() => window.showSaveFilePicker({ ...PICKER, suggestedName: (current && current.name) || 'patch.lsyn', startIn: current || 'documents' }));
    if (!h) return null;
    await write(h, text);
    await remember(h);
    return h.name;
