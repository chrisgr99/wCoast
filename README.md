...
The modules that ship today are a West Coast-style complex oscillator, a quad low-pass gate, a quad function generator, and a mixer — reflecting my interest in West Coast synthesis (Buchla and Serge style) — but nothing in the architecture ties LibreSynth to that: it's an open platform meant for modules of any kind.
...
LibreSynth is in alpha — fully usable, but expect rough edges and things to change. What works today:
...
Still ahead, designed but not yet built: the floating signal sources and injectors above, polyphony (it's one voice today), and an interface module that conforms to the Web Audio Modules (WAM) plugin standard, so other in-browser audio hosts and sequencers can drive LibreSynth and embed it as a sound source. Many other modules are planned; suggestions are welcome.

Share thoughts, bugs, and ideas in the [discussions](https://github.com/chrisgr99/LibreSynth/discussions).

## Running it

There's nothing to download or install. Open the GitHub Pages build at [chrisgr99.github.io/LibreSynth](https://chrisgr99.github.io/LibreSynth/) and follow the getting-started notes that appear on first run. A cross-platform desktop version, built on Electron, is coming in the near future.
...
**One caveat — turn off page-recolouring extensions.** If you use an add-on that changes how pages look (Dark Reader, or any dark-mode or colour-adjusting extension), disable it for LibreSynth. The app has its own light and dark modes, and these extensions distort the panels and lettering — the most likely culprit if anything ever looks wrong.
