# LibreModular — Design Document

LibreModular is an open modular synthesizer for Web Audio,
...
LibreModular is modelled on Buchla, which separates signals into domains. We use
...
Throughout LibreModular, **"module" ALWAYS means a complete Buchla module** (the 259t
...
Full-fidelity rule: each LibreModular module represents its real Buchla counterpart
...
- **Separate audio contexts.** GXW and LibreModular do NOT share an audio graph;
...
- **GXW is tick-rate and cannot produce audio-rate control.** This is settled
  ...
  - **All fast/audio-rate modulation lives inside LibreModular** (its function
...
- **Strudel is a co-client** (revised — see `design/control-protocol.md`). GXW,
  ...
  names onto the protocol’s fields, and LibreModular is the target it would otherwise
...
problems LibreModular does not have: autotuning oscillator **drift** (a digital
oscillator computes frequency exactly and never drifts), and **polyphonic
voice allocation over a hardware protocol** (handled by LibreModular's own voice
