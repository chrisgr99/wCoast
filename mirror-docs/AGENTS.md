# AGENTS.md — LibreSynth patch mirror

This folder is the **patch mirror** for a running LibreSynth session. LibreSynth is an open modular synthesizer for Web Audio, packaged as a macOS
...
patch. When you write it, LibreSynth validates the patch against `catalogue.json`,
...
- Write `patch.json` with an **atomic temp-and-rename**: write `patch.json.tmp`
  first, then rename it to `patch.json`. LibreSynth never sees a torn file, and the
...
- After writing, read `last-apply-result.json`:
  ...
  - `{ "status": "rejected", "error": "…" }` — nothing changed, and LibreSynth has
...
`isLive` in `active.json` is `true` while LibreSynth is running and `false` after it
...
- `patch.json` — the current patch: the modules placed, the wiring between them,
  and every parameter value. This is the same format LibreSynth saves to a `.wcoast`
